...
--
Можно и дальше решать это уравнение, но это не наш путь. Исследуем гамильтониан с целью упростить себе задачу.
...
--
Таким образом, $\psi_n$ также является собственной функцией оператора $a^\dagger a$, то есть собственные функции операторов совпадают, а собственные значения связаны соотношением
[env.equation]
--
\lambda_n = \frac{E_n}{\hbar\omega} - \frac{1}{2}.
--
Применим слева к обоим частям уравнения оператор $a$:
[env.equation]
--
...
--
То есть $a\psi_n$ тоже собственная функция с собственным значением $\lambda_n-1$. Теперь поменяем порядок операторов и провернув всё тот же трюк, получим
[env.equation]
--
a^\dagger a (a^\dagger\psi_n) = (\lambda_n+1) (a^\dagger\psi_n)
--
$a^\dagger\psi_n$ тоже собственная функция, но с собственным значением $\lambda_n-1$. Таким образом, узнав хотя бы одну собственную функцию мы можем построить целое семейство, применяя к ней операторы $a$ и $a^\dagger$.
...
--
Так как функция под интегралом всюду неотрицательна, то $\lambda_n \ge 0$. Предположим теперь, что существует собственная функция $\psi_k$, соответствующее нецелому значению $\lambda_k > 0$. Тогда последовательно применяя оператор $a$ к функции $\psi_k$ можно в определённый момент получить функцию
$\psi_m = a^{\lceil\lambda_k\rceil}\psi_k$ с собственным значением $\lambda_m = \lambda_k - \lceil\lambda_k\rceil < 0$. Но это противоречит условию неотрицательности собственых значений оператора $a^\dagger a$, поэтому $\lambda_n$ может принимать только целые неотрицательные значения. Начнём нумерацию с нуля и положим
[env.equation]
--
\lambda_n = n.
--
Тогда, $E_0=\hbar\omega/2$ является собственным значением гамильтониана. Найдём соответстующую собственную функцию $\psi_0$ из условия
[env.equation]
--
...
[env.equation]
--
C^2\int\exp\left(-2\frac{m \omega x^2}{\hbar}\right)\, dx = 1
--
получим значение константы $C$:
[env.equation]
--
C = \sqrt[4]{\frac{2m\omega}{\pi\hbar}}
--
